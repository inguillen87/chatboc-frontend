...
const SCENARIO_MATCHERS = [
  { predicate: (survey) => survey?.slug?.includes('ciudad-sustentable-2025'), scenario: 'mendoza-sustentable-2025' },
  { predicate: (survey) => survey?.slug?.includes('intencion-voto-municipal-2025'), scenario: 'mendoza-intencion-voto-2025' },
  { predicate: (survey) => survey?.slug?.includes('inflacion-hogar-2025'), scenario: 'mendoza-costo-vida-2025' },
  { predicate: (survey) => survey?.slug?.includes('agenda-hidrica-2026'), scenario: 'mendoza-agua-2026' },
];

const buildScenario = (name) => {
  const base = SCENARIO_PRESETS[name];
  if (!base) {
    return JSON.parse(JSON.stringify(BASE_SCENARIO));
  }
  const merged = {
    ...BASE_SCENARIO,
    ...base,
...
const pickClusterCoordinate = (scenario, override) => {
  if (override) {
    return {
      location: {
        lat: override.lat,
      ...
      radiusKm: override.radiusKm ?? 2.4,
    };
  }
...
const selectOptionsWithWeights = (pregunta, scenario) => {
...
const buildRandomAnswers = (survey, scenario, suggestionIndex) =>
  survey.preguntas.map((pregunta) => {
    ...
    return { pregunta_id: pregunta.id, opcion_ids: opcionIds };
  });
...
const run = async () => {
  try {
    ...
    const scenarioArg = args.scenario ?? args.profile ?? null;
    const municipioOverride = args.municipality ?? args.m ?? null;
    const dryRun = Boolean(args['dry-run'] ?? args.dry);

    const centerArg = args.center ?? args.latlng;
    const radiusOverride = toNumber(args.radius ?? args.r, Number.NaN);
    ...
    if (centerArg) {
      ...
      if (Number.isFinite(parsedLat) && Number.isFinite(parsedLng)) {
        centerOverride = {
          lat: parsedLat,
          lng: parsedLng,
          radiusKm: Number.isFinite(radiusOverride) ? radiusOverride : undefined,
        };
      }
    }
    ...
    let scenarioName = scenarioArg ? String(scenarioArg) : null;
    if (!scenarioName) {
      const matched = SCENARIO_MATCHERS.find((entry) => entry.predicate(survey));
      scenarioName = matched?.scenario ?? 'default';
    }

    const scenario = buildScenario(scenarioName);
    if (municipioOverride) {
      scenario.municipalityLabel = municipioOverride;
    }

    console.log(
      `Generando ${count} respuestas de demostración para "${survey.titulo}" usando el escenario "${scenario.label}"…`,
    );

    const results = [];

    for (let index = 0; index < count; index += 1) {
      const cluster = pickClusterCoordinate(
        scenario,
        centerOverride
          ? { ...centerOverride, barrio: scenario.municipalityLabel, ciudad: scenario.municipalityLabel }
          : null,
      );
      const radiusKm = Number.isFinite(radiusOverride)
        ? radiusOverride
        : cluster.radiusKm ?? (centerOverride?.radiusKm ?? 2.5);
        ...
        metadata: {
          ...
          demographics: {
            ...
            ubicacion: {
              ...
              origen: 'gps',
              pais: 'Argentina',
              provincia: cluster.location.provincia ?? 'Mendoza',
      ...
      if (dryRun) {
        results.push({ ok: true, preview: payload });
    ...
    if (dryRun) {
      console.log(`Se generarían ${ok} respuestas (${failed} no válidas). Mostrando vista previa de la primera:`);
      if (results[0]?.preview) {
        console.dir(results[0].preview, { depth: 4 });
      }
      return;
    }
