...
import {readBrandSnapshot,type BrandSnapshot,type BrandValues} from '@/utils/workspaceBranding';
export function useBrandStudio(slug:string,onPublished?:(brand:BrandSnapshot)=>void) {
  ...
  const publish=async(restoreVersion?:number)=>{
    if(!live.current||busy.current||needsReview||error||!snapshot?.can_edit||!draft)return;
    busy.current=true;setPending(true);setMessage(null);const seq=++generation.current;
  ...
  return {snapshot,draft,setDraft,pending,needsReview,latest,message,error,publish,chooseLatest,
