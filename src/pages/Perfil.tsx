...
const RUBRO_AVATAR: { [key: string]: string } = {
  bodega: "🍷", restaurante: "🍽️", almacen: "🛒", ecommerce: "🛍️", medico: "🩺", municipio: "🏛️", default: "🏢",
};
const PROVINCIAS = [
...
export default function Perfil() {
  ...
  const handleGuardar = async (e: FormEvent) => {
    ...
    try {
      ...
      // Opcional: Re-sincronizar para asegurar consistencia tras guardar
      // fetchPerfil(token);
    } catch (err: any) {
              ...
              <form onSubmit={handleGuardar} className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select id="provincia" value={perfil.provincia} onChange={handleInputChange} required className="w-full rounded-md border border-input bg-input px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary h-10">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    <Button type="button" variant={modoHorario === "comercial" ? "secondary" : "outline"} size="sm" onClick={setHorarioComercial} className="border-border hover:bg-accent">Automático (Lun-V 9-20)</Button>
                    <Button type="button" variant={modoHorario === "personalizado" ? "secondary" : "outline"} size="sm" onClick={setHorarioPersonalizado} className="border-border hover:bg-accent flex items-center">
                      {horariosOpen ? <ChevronUp className="w-4 h-4 mr-1" /> : <ChevronDown className="w-4 h-4 mr-1" />}
                      Personalizar
                    </Button>
                  </div>
                        ...
                        <div key={dia} className="grid grid-cols-[auto_1fr_auto] sm:grid-cols-[100px_auto_1fr] items-center gap-x-2 gap-y-1 text-sm">
                          ...
                          <div className="flex items-center gap-2 col-span-3 sm:col-span-1 sm:justify-self-end">
                            <Label htmlFor={`cerrado-${idx}`} className="text-xs text-muted-foreground flex items-center gap-1 cursor-pointer select-none">
                              <input type="checkbox" id={`cerrado-${idx}`} checked={perfil.horarios_ui[idx].cerrado} onChange={e => handleHorarioChange(idx, "cerrado", e.target.checked)} className="form-checkbox h-4 w-4 text-primary bg-input border-border rounded focus:ring-primary cursor-pointer"/> Cerrado
                            </Label>
                          </div>
                ...
                {mensaje && <div className="mt-3 text-sm text-green-700 bg-green-100 p-3 rounded-md flex items-center gap-2"><CheckCircle className="w-4 h-4"/> {mensaje}</div>}
  ...
  <div className="space-y-2 mt-3">
    ...
    >
      Mejorar a FULL ($60.000/mes)
            ...
            {/* Card Integración */}
             {/* El resto del código de la columna derecha no necesita cambios, lo puedes dejar como está... */}
            <Card className="bg-card shadow-xl rounded-xl border border-border backdrop-blur-sm">
              <CardHeader>
