// Contenido COMPLETO y MEJORADO para: TicketsPanel.tsx
...
interface Ticket {
    ...
    detalles?: string;
    comentarios?: Comment[];
    // Podríamos añadir más datos que vengan del backend
...
function fechaLarga(iso: string) {
    ...
    return d.toLocaleString('es-AR', { dateStyle: 'long', timeStyle: 'short' }) + ' hs';
}
...
export default function TicketsPanel() {
    ...
    const handleSelectTicket = useCallback(async (ticketSummary: TicketSummary) => {
        ...
        setIsModalOpen(true);
        try {
            // Esta es la llamada clave que trae los detalles completos del ticket
            ...
            setError(errorMessage); // Idealmente, mostrar este error en un "toast" o notificación
            setIsModalOpen(false);
        }
    }, []);

    // Esta función se llama desde TicketDetail para mantener todo sincronizado
                ...
                {Object.keys(categorizedTickets).length === 0 && !isLoading ? (
                    ...
                    Object.entries(categorizedTickets).map(([category, tickets]) => (
                        <TicketCategoryAccordion 
                            key={category} category={category} tickets={tickets}
                            onSelectTicket={handleSelectTicket} isOpen={openCategories.has(category)}
...
const TicketCategoryAccordion: FC<{
    category: string; tickets: TicketSummary[]; onSelectTicket: (ticket: TicketSummary) => void;
...
// --- SUB-COMPONENTE PARA EL DETALLE DEL TICKET (COMPLETAMENTE FUNCIONAL) ---
const TicketDetail: FC<{ ticket: Ticket; onTicketUpdate: (ticket: Ticket) => void }> = ({ ticket, onTicketUpdate }) => {
    ...
    const handleSendMessage = async () => {
        ...
        setIsSending(true);
        try {
            // ¡IMPORTANTE! Asegúrate de que tu API tenga este endpoint: POST /tickets/:id/comentar
            const updatedTicket = await apiFetch<Ticket>(`/tickets/${ticket.id}/comentar`, {
                method: 'POST',
                body: { comentario: newMessage },
            });
            onTicketUpdate(updatedTicket); // Actualiza la UI con la respuesta de la API
            setNewMessage("");
        } catch (error) {
    ...
    const handleEstadoChange = async (nuevoEstado: TicketStatus) => {
        try {
            // ¡IMPORTANTE! Asegúrate de que tu API tenga este endpoint: PUT /tickets/:id/estado
        ...
        <>
            <DialogHeader className="p-4 border-b border-border sticky top-0 bg-card z-10">
                <DialogTitle className="flex items-center gap-3">
                    ...
                    <span className="truncate">Ticket #{ticket.nro_ticket} - {ticket.asunto}</span>
                </DialogTitle>
                <DialogDescription className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm pt-2">
                    <span>Creado: {fechaLarga(ticket.fecha)}</span>
                    {ticket.nombre_usuario && <span>Por: {ticket.nombre_usuario}</span>}
                </DialogDescription>
            </DialogHeader>
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-3 overflow-hidden">
                    ...
                    <main className="flex-1 p-4 space-y-4 overflow-y-auto custom-scroll">
                        {ticket.comentarios?.map((comment) => (
                            <div key={comment.id} className={cn('flex items-end gap-2', comment.es_admin ? 'justify-end' : 'justify-start')}>
                                ...
                                <div className={cn("max-w-md rounded-lg px-4 py-2", 
                                    comment.es_admin 
                ...
                <aside className="md:col-span-1 border-l border-border bg-muted/30 p-4 space-y-6 overflow-y-auto custom-scroll">
                    <Card>
                        ...
                        <CardContent>
                            <Select onValueChange={handleEstadoChange} defaultValue={ticket.estado}>
                    ...
                    <Card>
                        ...
                        <CardContent className="text-sm text-muted-foreground space-y-1">
                           <p>{ticket.detalles || "No se proveyeron detalles adicionales."}</p>
                        </CardContent>
                    </Card>
                    {ticket.nombre_usuario && (
                        <Card>
                             ...
                             <CardContent className="text-sm text-muted-foreground space-y-1">
                                <p><strong>Nombre:</strong> {ticket.nombre_usuario}</p>
...
// Pequeño componente helper para los avatares del chat
