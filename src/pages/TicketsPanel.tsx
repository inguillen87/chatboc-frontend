...
import { Loader2, CheckCircle2, ArrowLeft, Send, Ticket as TicketIcon } from "lucide-react"; // Renombrado para evitar conflicto
...
export default function TicketsPanel() {
  ...
  const handleSelectTicket = (ticket: TicketType) => {
    ...
    if(isMobile) {
        // Lógica para vista mobile si es necesaria
    }
  };
      ...
      {showList && (
        <aside className="w-full md:w-1/3 md:max-w-[400px] bg-slate-50 dark:bg-slate-900 border-r dark:border-slate-800 flex flex-col">
          <header className="px-6 py-5 flex items-center gap-3 border-b dark:border-slate-800">
            <CheckCircle2 className="text-blue-500 w-6 h-6" /><h2 className="font-bold text-xl text-slate-800 dark:text-slate-100">Tickets de Vecinos</h2>
          </header>
...
const TicketList: FC<TicketListProps> = ({ tickets, selectedTicketId, onSelectTicket }) => (
  <div className="flex-1 overflow-y-auto">
    {tickets.length === 0 ? <p className="p-6 text-center text-slate-500">No hay tickets para mostrar.</p> :
      tickets.map((t) => (
        <div key={t.id}
          className={`p-4 flex flex-col gap-1 border-b dark:border-slate-800 cursor-pointer transition-colors hover:bg-blue-50 dark:hover:bg-blue-950/60 ${selectedTicketId === t.id ? "bg-blue-100 dark:bg-blue-900/70 border-l-4 border-blue-500 pl-3" : "border-l-4 border-transparent"}`}
...
const TicketDetail: FC<TicketDetailProps> = ({ ticket, userId, onBack }) => {
  ...
  const fetchComments = useCallback(async () => { /* ... tu lógica de fetchComments ... */ }, [ticket.id, userId]);
  useEffect(() => { fetchComments(); }, [fetchComments]);
  const handleSendMessage = async () => { /* ... tu lógica para enviar mensajes ... */ };

  return (
    <div className="flex flex-col h-full">
      <header className="flex items-center gap-3 p-4 border-b dark:border-slate-800">
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50 dark:bg-slate-900/50">
          <div className="pb-4 border-b dark:border-slate-700">
              <p className="whitespace-pre-wrap text-slate-800 dark:text-slate-200">{ticket.pregunta}</p>
          </div>
      ...
      <footer className="border-t p-3 flex gap-2 bg-white dark:bg-slate-950">
