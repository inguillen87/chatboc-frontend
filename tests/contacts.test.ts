...
describe('getSpecializedContact', () => {
  ...
  it('matches category ignoring case and accents', async () => {
    const mockData = { Arbolado: { nombre: 'Contacto Arbolado' }, default: { nombre: 'Genérico' } };
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: true, json: async () => mockData }));
    const { getSpecializedContact } = await loadModule();
    const contact = await getSpecializedContact('árbolado  ');
    expect(contact).toEqual(mockData.Arbolado);
  });
});
