...
import { getRuntimeRecoveryUI, loadRuntimeRecoveryUI } from '@/services/runtimeRecoveryConfig';
import type { RecoveryState, RuntimeRecoveryUI } from '@/config/runtimeRecovery';

export type RecoveryPhase = 'quiet' | RecoveryState;
...
export function useRuntimeRecovery() {
  ...
  const [ui, setUI] = useState<RuntimeRecoveryUI | null>(() => getRuntimeRecoveryUI());
  ...
  const check = useCallback((refresh = false, manual = false) => {
    ...
    const probe = async () => {
      const copy = getRuntimeRecoveryUI() ?? await loadRuntimeRecoveryUI();
      if (!active.current || current !== generation.current) return;
      if (!copy) { setUI(null); setPhase('quiet'); return; }
      setUI(copy);
      await ensureBackendRuntimeReady({ enabled: true, refresh });
  ...
  useEffect(() => {
    active.current = true;
    const installed = generation.current;
    if (isRuntimeRecoveryEnabled() && online() && document.visibilityState !== 'hidden') {
      void loadRuntimeRecoveryUI().then(copy => {
        if (active.current && generation.current === installed && isRuntimeRecoveryEnabled()) setUI(copy);
      });
    }
    const disconnected = () => {
  ...
  return { phase, ui, retry: () => check(true, true), dismiss: () => setPhase(current => current === 'verified' ? 'quiet' : current) };
}
