export type ModuleId='whatsapp'|'catalog'|'payments'|'surveys'|'territory';
...
const ids:ModuleId[]=['whatsapp','catalog','payments','surveys','territory'];
...
export function readModuleSelection(value:unknown,slug:string):ModuleSelection|null {
  if(!object(value)||value.contract_version!=='organization.setup_modules.v1'||value.catalog_version!==1
  ...
  const expectedIds=ids.filter(id=>id!=='territory'||['municipio','gobierno'].includes(value.organization_type));
  if(!Array.isArray(value.catalog)||value.catalog.length!==expectedIds.length)return null;
  for(const [i,module] of value.catalog.entries()) {
    if(!object(module)||module.id!==expectedIds[i]||!text(module.label)||!text(module.description)
      ||!Array.isArray(module.requires)||!equalModules(module.requires,module.id==='payments'?['catalog']:[]))return null;
  }
  const ui=readModuleUI(value.ui);
  if(!ui||!validModules(value.selected,value.catalog)||!equalModules(value.selected,value.catalog.filter((m:ModuleDefinition)=>value.selected.includes(m.id)).map((m:ModuleDefinition)=>m.id))
  ...
  return {...value,ui} as ModuleSelection;
}
