...
const STATUS_CONFIG: Record<string, { label: string; color: string; accent: string; icon: any; step: number; description: string }> = {
  pendiente: {
    label: 'Recibido',
...
const InfoMetric = ({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) => (
...
const EmptyConversationState = () => (
  <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50/80 px-6 py-10 text-center">
    ...
    <p className="text-sm font-semibold text-slate-900">Todavía no hay mensajes públicos</p>
    <p className="mt-2 text-sm text-slate-500">Cuando exista una respuesta o comentario sobre este reclamo, lo vas a ver en esta conversación.</p>
  </div>
);
...
export default function TicketLookup() {
  ...
  const [publicAccessSource, setPublicAccessSource] = useState<'manual' | 'url' | 'restored'>('manual');

  const storedPublicAccess = useMemo(() => readStoredPublicAccess(), [ticketId]);
  const currentPin = useMemo(() => {
    ...
    if (ticketId && storedTicketId && storedTicketId === ticketId && typeof storedPublicAccess?.pin === 'string') {
  ...
  useEffect(() => {
    ...
    const storedTicketId = String(storedPublicAccess?.ticketId || '');
    if (ticketId && storedTicketId === ticketId && typeof storedPublicAccess?.pin === 'string' && storedPublicAccess.pin.trim()) {
  ...
  const syncPublicAccess = useCallback((resolvedTicket: Ticket | null, pin: string) => {
    if (!resolvedTicket?.id || !pin) return;
    const anonId = getOrCreateAnonId();
    safeLocalStorage.setItem('ticket_public_access', JSON.stringify({
  ...
  const loadConversationData = useCallback(async (resolvedTicket: Ticket, pin: string, silent = false) => {
    if (!silent) {
      setRefreshingConversation(true);
    }
    try {
      const [timeline, messages] = await Promise.all([
        getTicketTimeline(resolvedTicket.id, resolvedTicket.tipo || 'municipio', {
          public: true,
          pin,
        }).catch((msgErr) => {
          console.warn('Error fetching timeline', msgErr);
          return { estado_chat: '', history: resolvedTicket.history || [], messages: [] };
        }),
        getTicketMessages(resolvedTicket.id, resolvedTicket.tipo || 'municipio', {
          public: true,
          pin,
        }).catch((msgErr) => {
  ...
  const performSearch = useCallback(async (searchId?: string, searchPin?: string) => {
    ...
    try {
      ...
      const normalizedTicket = { ...data, history: data.history || [], messages: data.messages || [] };
  ...
  const handleOpenChat = () => {
    ...
    safeLocalStorage.setItem('pending_widget_action', JSON.stringify({
      ...
      text: ticket?.nro_ticket ? `Seguimiento de reclamo #${ticket.nro_ticket}` : undefined,
    }));
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      await sendMessage(ticket.id, ticket.tipo || 'municipio', message.trim(), undefined, undefined, {
        public: true,
        pin: currentPin,
      });
  ...
  const ticketLat = pickCoordinate(ticket?.latitud, ticket?.lat_destino, ticket?.lat_actual);
  const ticketLng = pickCoordinate(ticket?.longitud, ticket?.lon_destino, ticket?.lon_actual);
  const municipalityLat = pickCoordinate(ticket?.municipio_latitud, ticket?.origen_latitud, ticket?.lat_origen);
  const municipalityLng = pickCoordinate(ticket?.municipio_longitud, ticket?.origen_longitud, ticket?.lon_origen);
  const hasCoordinates = ticketLat !== null && ticketLng !== null;
  const hasStoreCoordinates = municipalityLat !== null && municipalityLng !== null;
        ...
        .then(async (data) => {
          setTicket(prev => {
            ...
            if (prev.estado !== data.estado || (prev.history?.length !== data.history?.length)) {
              return { ...prev, ...data, history: data.history || prev.history || [] };
            }
            return prev;
          });
  ...
  }, [ticket, isResolved, inputTicketId, ticketId, currentPin, loadConversationData, syncPublicAccess]);
        ...
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-slate-900">Atención Ciudadana</p>
              <p className="truncate text-xs text-slate-500">Seguimiento seguro de reclamos</p>
            </div>
          </div>
          {ticket && (
            <Button variant="ghost" size="sm" onClick={() => setIsSupportOpen(true)} className="gap-2 text-blue-700 hover:bg-blue-50 hover:text-blue-800">
        ...
        {!ticket && (
          <div className="mx-auto max-w-lg space-y-6 py-10">
            <div className="space-y-3 text-center">
              ...
              <h1 className="text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">Estado de Reclamo</h1>
              <p className="mx-auto max-w-md text-sm leading-6 text-slate-500 sm:text-base">Ingresá el número de gestión y tu PIN de seguridad para ver el estado, la conversación y la ubicación reportada.</p>
            </div>
              ...
              <CardContent className="space-y-6 pt-6">
                <form onSubmit={handleSearchSubmit} className="space-y-4">
                  <div className="space-y-2 text-left">
                    <label className="text-sm font-medium text-slate-700">Número de Reclamo</label>
                  ...
                  <div className="space-y-2 text-left">
                    <label className="text-sm font-medium text-slate-700">PIN de Seguridad</label>
                    <Input
                      placeholder="••••"
                      type="password"
                  ...
                  <Button type="submit" className="h-11 w-full rounded-xl bg-blue-600 text-white shadow-lg shadow-blue-500/25 hover:bg-blue-700" disabled={loading || !inputTicketId || !inputPin}>
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  <InfoMetric icon={ShieldCheck} label="Acceso" value="Protegido con PIN" />
                  <InfoMetric icon={MessagesSquare} label="Canales" value="Estado y conversación" />
                  <InfoMetric icon={MapPin} label="Ubicación" value="Mapa y dirección" />
                </div>
                {publicAccessSource === 'restored' && currentPin ? (
                  <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-700">
                    Recuperamos tu acceso seguro para este ticket desde tu sesión actual.
                  </div>
                ) : null}
              </CardContent>
            </Card>
        ...
        {ticket && (
          <>
            <Card className="overflow-hidden border-0 bg-white/85 shadow-2xl shadow-slate-200/60 ring-1 ring-black/5 backdrop-blur">
              ...
              <CardContent className={`relative overflow-hidden bg-gradient-to-br ${statusInfo.accent} px-6 pb-8 pt-6 text-center`}>
                <div className="absolute inset-x-0 top-0 h-32 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.95),_transparent_70%)]" />
                <div className="relative">
                  <div className={`mb-6 inline-flex rounded-3xl border px-4 py-3 shadow-sm ${statusInfo.color}`}>
                  ...
                  <div className="mb-4 flex flex-wrap items-center justify-center gap-2">
                    ...
                    <div className="inline-flex items-center gap-1.5 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-slate-600 shadow-sm ring-1 ring-slate-200/80">
                      <RefreshCw className={`h-3.5 w-3.5 ${refreshingConversation ? 'animate-spin' : ''}`} />
                      {formatLastSync(lastSyncedAt)}
                    </div>
                  </div>
                  <h1 className="text-3xl font-bold text-slate-950">{statusInfo.label}</h1>
                  <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-600 sm:text-base">{statusInfo.description}</p>

                  <div className="mx-auto mt-8 grid max-w-3xl gap-3 sm:grid-cols-3">
                    <InfoMetric icon={Calendar} label="Creado" value={formatShortDate(ticket.fecha_creacion)} />
                    <InfoMetric icon={MessagesSquare} label="Conversación" value={publicMessagesCountLabel} />
                    <InfoMetric icon={Sparkles} label="Timeline" value={timelineCountLabel} />
                  </div>
                        ...
                        {TICKET_STEPS.map((step, index) => {
                          ...
                          return (
                            <div key={step.id} className="flex flex-col items-center gap-3">
                              <div className={`flex h-8 w-8 items-center justify-center rounded-full border-2 bg-white transition-all duration-500 ${isCompleted ? 'border-blue-600 text-blue-600 shadow-sm' : 'border-slate-200 text-slate-300'} ${isCurrent ? 'scale-110 ring-4 ring-blue-100' : ''}`}>
                                {isCompleted ? <CheckCircle2 className="h-4 w-4" /> : <div className="h-2 w-2 rounded-full bg-current" />}
                              </div>
                              <span className={`max-w-[70px] text-center text-[10px] font-semibold uppercase tracking-wide sm:text-xs ${isCompleted ? 'text-slate-900' : 'text-slate-400'}`}>
                                {step.label}
                              </span>
                            </div>
                          );
                ...
                <Card className="border-0 bg-white/85 shadow-lg shadow-slate-200/50 ring-1 ring-black/5 backdrop-blur">
                  <CardHeader className="border-b border-slate-100 pb-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        ...
                        <CardDescription>Información principal asociada a esta gestión.</CardDescription>
                      </div>
                      <button type="button" onClick={copyToClipboard} className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-100">
                        #{ticket.nro_ticket}
                  ...
                  <CardContent className="space-y-6 pt-6">
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                      <InfoMetric icon={FileText} label="Categoría" value={ticket.categoria || 'General'} />
                      <InfoMetric icon={ShieldCheck} label="Estado" value={statusInfo.label} />
                      <InfoMetric icon={Calendar} label="Alta" value={formatShortDate(ticket.fecha_creacion)} />
                    </div>

                    {ticket.description && (
                      <div>
                        <h3 className="mb-2 text-sm font-medium text-slate-500">Descripción</h3>
                    ...
                    {primaryImageUrl && (
                      <div>
                        ...
                        <div className="group relative aspect-video w-full cursor-pointer overflow-hidden rounded-2xl border border-slate-200 bg-slate-100" onClick={() => window.open(primaryImageUrl, '_blank')}>
                          <img src={primaryImageUrl} alt="Evidencia" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
                          <div className="absolute inset-0 flex items-center justify-center bg-black/0 transition-colors group-hover:bg-black/20">
                            <span className="rounded-full bg-black/60 px-3 py-1 text-sm font-medium text-white opacity-0 backdrop-blur-sm transition-opacity group-hover:opacity-100">Ver imagen completa</span>
                          </div>
                        </div>
                      </div>
                    )}
                ...
                <Card className="border-0 bg-white/85 shadow-lg shadow-slate-200/50 ring-1 ring-black/5 backdrop-blur">
                  <CardHeader className="border-b border-slate-100 pb-4">
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        <CardTitle className="text-lg text-slate-950">Historial de Actividad</CardTitle>
                        <CardDescription>Timeline unificada del reclamo.</CardDescription>
                      </div>
                      <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-500">
                        {refreshingConversation ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}
                        {formatLastSync(lastSyncedAt)}
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="pt-6">
                    <div className="relative space-y-6 pl-2">
                      ...
                      {timelineHistory.map((event, i) => (
                        <div key={`${event.date}-${event.status}-${i}`} className="relative flex gap-4">
                          ...
                          <div className="flex-1 rounded-2xl border border-slate-100 bg-slate-50/60 px-4 py-3 shadow-sm">
                            <p className="text-sm font-medium text-slate-900">{event.notes || event.status}</p>
                            <p className="mt-1 text-xs text-slate-500">{formatEventDate(event.date)}</p>
                          </div>
                        </div>
                      ))}
                  ...
                  <CardHeader className="border-b border-slate-100 pb-4">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      <div>
                        ...
                        <CardDescription>Mensajes públicos asociados a este ticket.</CardDescription>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500">{publicMessagesCountLabel}</div>
                        <Button variant="outline" size="sm" className="rounded-full" onClick={() => setIsSupportOpen(true)}>
                          Escribir
                        </Button>
                      </div>
                    </div>
                  ...
                  <CardContent className="space-y-4 pt-6">
                    {publicMessages.length === 0 ? (
                      ...
                      publicMessages.map((publicMessage) => {
                        ...
                        return (
                          <div key={publicMessage.id} className={`flex ${isAgent ? 'justify-start' : 'justify-end'}`}>
                            <div className={`max-w-[90%] rounded-[24px] px-4 py-3 shadow-sm sm:max-w-[80%] ${isAgent ? 'border border-slate-200 bg-white text-slate-900' : 'bg-gradient-to-br from-blue-600 to-indigo-600 text-white'}`}>
                              <div className="mb-2 flex items-center gap-2">
                                <div className={`h-2.5 w-2.5 rounded-full ${isAgent ? 'bg-emerald-500' : 'bg-blue-100'}`} />
                                <span className={`text-[11px] font-semibold uppercase tracking-[0.12em] ${isAgent ? 'text-slate-500' : 'text-blue-100'}`}>
                                  {isAgent ? 'Atención' : 'Tu mensaje'}
                                </span>
                              </div>
                              <p className="whitespace-pre-wrap break-words text-sm leading-6">{publicMessage.content}</p>
                              <p className={`mt-2 text-[11px] ${isAgent ? 'text-slate-400' : 'text-blue-100'}`}>{formatMessageDate(publicMessage.timestamp)}</p>
                            </div>
                          </div>
                        );
                      })
              ...
              <div className="space-y-6">
                <Card className="overflow-hidden border-0 bg-white/85 shadow-lg shadow-slate-200/50 ring-1 ring-black/5 backdrop-blur">
                  <div className="h-56 w-full bg-slate-100 relative">
                    {hasAnyCoordinates ? (
                      <TrackingMap status={currentStatusKey} storeLocation={mapStoreLocation} customerLocation={mapCustomerLocation} />
                    ) : (
                      <div className="flex h-full w-full flex-col items-center justify-center gap-3 bg-slate-50 px-6 text-center text-slate-400">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-slate-700">{hasLocationData ? 'Ubicación pendiente de geocodificación' : 'Sin ubicación reportada'}</p>
                          <p className="mt-1 text-xs text-slate-500">La dirección y las coordenadas se mostrarán acá cuando estén disponibles.</p>
                        </div>
                      </div>
                    )}
                  </div>
                  <CardHeader className="border-b border-slate-100 pb-4">
                    ...
                    <CardDescription>Fuente de verdad del ticket para el seguimiento público.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4 pt-6">
                    <InfoMetric icon={MapPin} label="Dirección" value={ticket.direccion || 'No especificada'} />
                    {hasCoordinates ? (
                    ...
                    {mapLink && (
                      <Button asChild variant="outline" className="h-11 w-full rounded-xl">
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          onClick={() => trackFrontendEvent('tracking_public_maps_opened', {
                            ticket_id: ticket.id,
                            ticket_number: ticket.nro_ticket,
                ...
                <Card className="overflow-hidden border-0 bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-600 text-white shadow-xl shadow-blue-500/25">
                  <CardContent className="relative p-6">
                    ...
                    <div className="relative space-y-4">
                      ...
                      <div>
                        <h3 className="text-xl font-bold">¿Necesitás ayuda con este reclamo?</h3>
                        <p className="mt-2 text-sm leading-6 text-blue-50">Podés dejar una observación en este mismo ticket o abrir el canal de atención en vivo sin salir del seguimiento.</p>
                      </div>
                      <div className="space-y-2">
                        <Button className="h-11 w-full rounded-xl bg-white text-blue-700 shadow-sm hover:bg-blue-50" onClick={() => setIsSupportOpen(true)}>
                          Escribir sobre este reclamo
                        </Button>
                        <Button variant="outline" className="h-11 w-full rounded-xl border-white/30 bg-white/10 text-white hover:bg-white/15 hover:text-white" onClick={handleOpenChat}>
                          Abrir chat en vivo
                        </Button>
                      </div>
                    </div>
                ...
                <Card className="border-0 bg-white/85 shadow-lg shadow-slate-200/50 ring-1 ring-black/5 backdrop-blur">
                  <CardContent className="space-y-4 p-6">
                    ...
                    />
                    <InfoMetric icon={RefreshCw} label="Sincronización" value={formatLastSync(lastSyncedAt)} />
                    <InfoMetric icon={Hash} label="Número" value={ticket.nro_ticket} />
                    <Button variant="link" className="h-auto justify-start px-0 text-xs text-slate-400" onClick={() => {
                      setTicket(null);
                      setTimelineHistory([]);
                      setPublicMessages([]);
                      setInputTicketId('');
      ...
      <Dialog open={isSupportOpen} onOpenChange={setIsSupportOpen}>
        <DialogContent className="overflow-hidden border-0 bg-white p-0 shadow-2xl sm:max-w-lg">
          <div className="border-b border-slate-100 bg-gradient-to-r from-slate-50 via-white to-blue-50 px-6 py-5">
            <DialogHeader>
              <DialogTitle className="text-left text-xl text-slate-950">Mesa de Ayuda</DialogTitle>
          ...
          <div className="grid gap-5 px-6 py-6">
            <Button variant="outline" className="group h-auto justify-start gap-4 rounded-2xl border-slate-200 px-4 py-4 transition-all hover:border-blue-500/30 hover:bg-slate-50" onClick={handleOpenChat}>
              ...
              <div className="text-left">
                ...
                <div className="text-xs text-slate-500">Abrí la atención en tiempo real del reclamo</div>
              </div>
            ...
            <div className="space-y-3">
              <label className="text-sm font-medium text-slate-700">Dejar un comentario / observación</label>
              <Textarea
              ...
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs text-slate-500">Tu mensaje quedará asociado a este ticket público.</p>
                <Button className="rounded-xl bg-blue-600 hover:bg-blue-700" onClick={handleSendMessage} disabled={!message.trim() || submittingPublicMessage}>
                  {submittingPublicMessage ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                  Enviar Mensaje
                </Button>
              </div>
            </div>
