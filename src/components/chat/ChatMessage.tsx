...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  const handleButtonClick = (payload: string) => {
    // Dispara un evento global que ChatPage escucha para enviar el mensaje.
      ...
      >
        ...
        {isBot && message.botones && message.botones.length > 0 && (
          <div className="mt-3 pt-3 border-t border-black/10 dark:border-white/10 flex flex-wrap gap-2">
            {message.botones.map((boton, index) => (
              <button
                key={index}
                onClick={() => handleButtonClick(boton.payload || boton.title)}
                className="px-3 py-1 bg-white/80 dark:bg-blue-800/90 backdrop-blur-sm text-blue-800 dark:text-blue-100 text-sm rounded-full hover:bg-white dark:hover:bg-blue-800 transition-all shadow-sm"
              >
                {boton.title}
              </button>
            ))}
        ...
        <div className={`text-xs mt-2 opacity-60 text-right select-none tracking-tight font-normal ${isBot ? "text-blue-700 dark:text-blue-200" : "text-white dark:text-gray-200"}`}>
