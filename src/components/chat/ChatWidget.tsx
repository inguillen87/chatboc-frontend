// Archivo: ChatWidget.tsx (versión final optimizada de Marcelo + JSX completo restaurado)
...
const ChatWidget = ({
  ...
  return (
    <div ref={widgetContainerRef} style={currentPos} className="chatboc-standalone-widget">
      {!isOpen && (
        <button onClick={toggleChat} className="group w-16 h-16 rounded-full flex items-center justify-center border shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:scale-105 bg-card border-border" aria-label="Abrir chat">
      ...
      {isOpen && (
        <div className="w-80 md:w-96 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-slide-up bg-white border border-border dark:bg-gray-900"
          ...
          {esperandoRubro ? (
            <div className={`w-full flex flex-col items-center justify-center p-6 text-foreground border border-border rounded-3xl
              ${isMobile ? "bg-white shadow-2xl dark:bg-gray-900" : "bg-card"}`}
              style={{ minHeight: 240 }}>
              ...
              {cargandoRubros ? (
                ...
                <div className="text-center text-destructive text-sm my-6">
                  ...
                  <button onClick={cargarRubros} className="mt-2 underline text-primary hover:text-primary/80">Reintentar</button>
                </div>
              ) : (
