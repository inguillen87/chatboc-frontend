  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 border-b border-border/70 pb-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Analítica descriptiva</p>
          <h3 id="ticket-stats-sparse-title" className="mt-1 text-lg font-semibold text-foreground">
            Distribución de la evidencia disponible
          </h3>
          <p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">
            Con este volumen se muestran valores exactos y proporciones, sin ejes que puedan exagerar diferencias pequeñas.
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        {charts.map((chart, chartIndex) => {
          ...
          return (
            <article key={`${chart.title}-${chartIndex}`} className="rounded-lg border border-border/70 bg-background p-4">
      ...
      <p className="mt-4 text-xs leading-5 text-muted-foreground">
        Lectura operativa, no inferencial: se ampliarán comparaciones y tendencias cuando la muestra alcance volumen suficiente.
      </p>
    </section>
  );
}

function PrivacyProtectedChartSummary({ privacyFloor }: { privacyFloor?: number }) {
  return (
    <section
    ...
    >
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Analítica protegida</p>
      <h3 className="mt-1 text-lg font-semibold text-foreground">Desglose reservado por privacidad</h3>
      <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
        La muestra visible todavía no alcanza el umbral requerido para publicar categorías, estados o zonas sin aumentar el riesgo de reidentificación.
      </p>
...
export default function TicketStatsCharts({
  ...
  if (typeof sampleSize === 'number' && sampleSize >= 0 && sampleSize < sparseThreshold) {
    return (
      <SparseChartSummary
        charts={charts}
