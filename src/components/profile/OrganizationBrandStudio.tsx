...
function BrandEditor({tenantSlug,name,logoUrl,onPublished}:Props){
  ...
  const [confirm,setConfirm]=React.useState<number|'publish'|null>(null);
  ...
  return <section className={styles.panel} aria-label="Estudio de marca" data-testid="brand-studio">
    ...
    <div className={styles.grid}><div className={styles.controls}>
      ...
      <div className={styles.ratios}><span>Principal: {primary.contrast.toFixed(2)}:1</span><span>Acento: {accent.contrast.toFixed(2)}:1</span></div>
      ...
      <div className={styles.actions}><button type="button" className={styles.primaryAction} disabled={actionDisabled} onClick={()=>setConfirm('publish')}>
        {pending?'Verificando…':'Publicar paleta'}</button>
        <button type="button" disabled={pending} onClick={state.refresh}><RefreshCw size={16} aria-hidden="true"/>Revisar versión actual</button></div>
      {state.latest?<div className={styles.review} aria-label="Comparar paletas">
        <p>Versión guardada {state.latest.version}: {state.latest.values.primary_color} · {state.latest.values.accent_color}</p>
        <p>Tu borrador: {draft.primary_color} · {draft.accent_color}. Elegir no publica.</p>
      ...
      <div className={styles.preview} data-theme={mode} data-mobile={mobile} style={{'--sample-brand':primary.background,'--sample-on-brand':primary.foreground,
        '--sample-accent':accent.background,'--sample-on-accent':accent.foreground} as React.CSSProperties}>
    ...
    <details className={styles.history}><summary><History size={17} aria-hidden="true"/>Historial de paleta · {snapshot.history.length} versiones anteriores</summary>
      {snapshot.history.length?snapshot.history.map(entry=><div key={entry.version}><span>Versión {entry.version} · {entry.values.primary_color} · {entry.values.accent_color}{entry.values.enabled?'':' · Desactivada'}</span>
    ...
    <AlertDialog open={confirm!==null} onOpenChange={open=>{if(!open)setConfirm(null);}}><AlertDialogContent>
      <AlertDialogHeader><AlertDialogTitle>{confirm==='publish'?'¿Publicar esta paleta?':`¿Restaurar la versión ${confirm}?`}</AlertDialogTitle>
        <AlertDialogDescription>Organización: {name||tenantSlug}. Cambiarán únicamente los colores de las superficies indicadas. No se envían mensajes ni se modifican planes o dominios.</AlertDialogDescription></AlertDialogHeader>
      <AlertDialogFooter><AlertDialogCancel>Seguir revisando</AlertDialogCancel><AlertDialogAction disabled={actionDisabled} onClick={()=>{
        const selected=confirm;setConfirm(null);void state.publish(typeof selected==='number'?selected:undefined);
      }}>Confirmar publicación</AlertDialogAction></AlertDialogFooter>
