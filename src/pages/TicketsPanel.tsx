// --- src/components/TicketsPanel.tsx (VERSIÓN FINAL, COMPLETA Y CORREGIDA) ---
...
export default function TicketsPanel() {
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      ...
      else throw new Error(data.error || "No se pudieron cargar los tickets.");
    } catch (err: unknown) {
    ...
    <div className="flex h-[calc(100vh-80px)] max-w-6xl mx-auto my-8 shadow-2xl rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-white">
      ...
      {showList && (
        <aside className="w-full md:w-1/3 md:max-w-[400px] bg-slate-50 dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col">
          ...
          {isLoading ? <div className="p-4"><p>Cargando tickets...</p></div> : 
      ...
      {tickets.map((t) => (
         <motion.div key={t.id} layoutId={`ticket-container-${t.id}`}
            ...
            <div className="flex justify-between items-center">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold text-white shadow ${ESTADOS[t.estado].badge}`}>{ESTADOS[t.estado].label}</span>
            </div>
...
const TicketDetail: FC<TicketDetailProps> = ({ ticket, userId, onBack, onTicketUpdate }) => {
    ...
    const [currentStatus, setCurrentStatus] = useState<TicketStatus>(ticket.estado);
    const chatBottomRef = useRef<HTMLDivElement>(null);
    const isMobile = useIsMobile();
    
    const fetchComments = useCallback(async () => {
        const data = await ticketService.getComments(ticket.id, userId);
        if (data.ok) setComments(data.comentarios);
        setIsLoading(false);
    }, [ticket.id, userId]);
    ...
    const handleSendMessage = async () => {
        ...
        setIsSending(true);
        await ticketService.postComment(ticket.id, userId, newMessage);
        setNewMessage("");
        await fetchComments();
        setIsSending(false);
    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-slate-950">
            <header className="flex items-center gap-3 p-5 border-b border-slate-200 dark:border-slate-800">
                ...
                {/* Aquí podrías añadir el dropdown para cambiar el estado si lo necesitas */}
            </header>
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-4">
                ...
                {isLoading ? <p>Cargando comentarios...</p> : comments.map(c => (
