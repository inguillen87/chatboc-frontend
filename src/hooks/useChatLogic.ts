...
export function useChatLogic({
  ...
  const sanitizeRubroValue = (value: unknown): string | null => {
    const info = extractRubroInfo(value);
    return info.clave ?? info.nombre;
  };
  ...
  const initializeConversation = useCallback(
    async (options?: {
      ...
      if (!rawRubro) {
        ...
        rawRubro = stored.clave ?? stored.nombre ?? null;
      }
      ...
      const rubroForPayload = tipoChatFinal === 'pyme' ? normalizedRubro || rawRubro : null;
    ...
    try {
      ...
      const payloadKeys =
        actionPayload && typeof actionPayload === 'object'
          ? Object.keys(actionPayload)
          : [];
      const payloadHasRubroKey = payloadKeys.some((key) => key.toLowerCase().includes('rubro'));
      const actionMentionsRubro = !!normalizedAction && normalizedAction.includes('rubro');

      let storedRubro: string | null = null;

      if (payloadHasRubroKey || actionMentionsRubro) {
        let payloadInfo = extractRubroInfo(actionPayload, {
          allowPlainString: typeof actionPayload === 'string' || payloadHasRubroKey,
          preferRubroKeys: true,
        });

        if (!payloadInfo.clave && !payloadInfo.nombre) {
          const fallbackSource = payloadNombre || (typeof actionPayload === 'string' ? actionPayload : null) || userMessageText || null;
          payloadInfo = extractRubroInfo(fallbackSource);
        }

        if (payloadInfo.clave || payloadInfo.nombre) {
          const persisted = persistRubroSelection(payloadInfo);
          storedRubro = persisted.clave ?? persisted.nombre ?? null;
        }
      }

      if (!storedRubro) {
        const userRubro = extractRubroInfo(storedUser?.rubro);
        storedRubro = userRubro.clave ?? userRubro.nombre ?? null;
      }

      if (!storedRubro) {
        const storedInfo = getStoredRubroInfo();
        storedRubro = storedInfo.clave ?? storedInfo.nombre ?? null;
      }

      const normalizedStoredRubro = storedRubro ? parseRubro(storedRubro) : null;
      const tipoChatFinal = enforceTipoChatForRubro(tipoChat, normalizedStoredRubro || undefined);
      const rubro = tipoChatFinal === 'pyme' ? normalizedStoredRubro || storedRubro : null;
