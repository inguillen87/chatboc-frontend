...
const TicketActions: React.FC<{ ticket: Ticket; onTicketPropertyChange?: TicketDetailPanelProps['onTicketPropertyChange'] }> = ({ ticket, onTicketPropertyChange }) => (
    <div className="p-2 border-b flex items-center justify-start gap-2 bg-background dark:bg-muted/30">
        <Select
            value={ticket.estado}
        ...
        <Button variant="outline" size="sm" className="h-8 text-xs">Derivar</Button>
    </div>
);
...
const TicketDetailPanel: React.FC<TicketDetailPanelProps> = ({ ticket, onTicketPropertyChange }) => {
  ...
  if (!ticket) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-4 bg-muted/20 dark:bg-background">
        <TicketIcon className="h-24 w-24 text-muted-foreground/30" />
        <h2 className="mt-6 text-xl font-semibold">Seleccione un Ticket</h2>
        <p className="text-base text-muted-foreground mt-1">Elija un ticket de la lista para ver los detalles.</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Tabs defaultValue="chat" className="flex-1 flex flex-col">
        <div className="px-3 py-2 border-b bg-background dark:bg-muted/30">
            <TabsList>
