...
import type { SurveyGovernanceRelease, SurveyGovernanceReleaseList } from '@/types/encuestas';
...
import { SurveyGovernancePanel } from './SurveyGovernancePanel';
...
describe('SurveyGovernancePanel', () => {
  ...
  it('fails closed when the backend omits explicit manage authority', async () => {
    apiMocks.list.mockResolvedValueOnce(
      listContract({
        capabilities: { read: true, manage: false, create_release: true },
        items: [],
        total: 0,
      }),
    );
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(/autorización explícita y verificable/i);
  ...
  it('does not trust an action boolean attached to a malformed release receipt', async () => {
    ...
    expect(await screen.findByText(/contrato, hashes, assurance o consentimiento público incompletos/i)).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /Publicar snapshot/i })).not.toBeInTheDocument();
  ...
  it('publishes and closes only when each release action is explicitly authorized', async () => {
    ...
    apiMocks.close.mockResolvedValueOnce({
      ...closed,
      idempotency: { persisted: true, replayed: false, disposition: 'accepted' },
    });
