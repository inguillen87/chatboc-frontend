...
export default function SuperadminFollowUpQueue(){
  ...
  return <section className="followup-queue" aria-label="Agenda de próximos contactos">
    ...
    {data && (counts.overdue > 0 || counts.today > 0 || counts.unverified > 0) ? (
      <div className="followup-attention" role="status" aria-label="Atención requerida">
        <div>
          <strong>Atención requerida</strong>
          <p>{counts.overdue} vencido{counts.overdue===1?'':'s'} · {counts.today} para hoy · {counts.unverified} con fecha no verificable.</p>
        </div>
        <div className="followup-attention-actions">
          {counts.overdue>0 ? <Button type="button" variant="outline" onClick={()=>setView('overdue')}>Ver vencidos</Button> : null}
          {counts.today>0 ? <Button type="button" variant="outline" onClick={()=>setView('today')}>Ver hoy</Button> : null}
          {counts.unverified>0 ? <Button type="button" variant="outline" onClick={()=>setView('unverified')}>Revisar fechas</Button> : null}
        </div>
      </div>
    ) : null}
    ...
    {loading?<p role="status">Consultando agenda…</p>:error?<p role="alert" className="followup-warning">{error}</p>:<>
      ...
      {!visible.length?<div className="followup-empty"><p>No hay contactos que coincidan con estos filtros.</p>{(view!=='all'||query.trim())?<Button type="button" variant="outline" onClick={()=>{setView('all');setQuery('');}}>Limpiar búsqueda y prioridad</Button>:null}</div>:<ul className="followup-rows">{visible.map(row=><li key={row.key}>
