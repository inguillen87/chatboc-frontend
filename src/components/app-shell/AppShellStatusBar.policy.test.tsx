...
describe('disabled recovery visibility', () => {
  it('does not display offline state when initially offline and disabled', () => {
    ...
    expect(screen.queryByTestId('runtime-recovery-bar')).toBeNull(); expect(policy.run).not.toHaveBeenCalled();
  });
  ...
  it('hides an existing offline state when the current route is excluded', () => {
    ...
    policy.enabled = false; event('offline'); expect(screen.queryByTestId('runtime-recovery-bar')).toBeNull();
  });
  it('performs no reconnect probe after an excluded offline event', async () => {
    ...
    expect(policy.run).not.toHaveBeenCalled(); expect(screen.queryByTestId('runtime-recovery-bar')).toBeNull();
  });
});
