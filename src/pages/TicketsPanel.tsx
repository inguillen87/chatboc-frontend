...
export default function TicketsPanel() {
  ...
  useEffect(() => {
    ...
    if (userRaw) {
      try {
        ...
        // Verificamos que el usuario y su ID existan
        if (user && user.id) {
          setUserId(user.id);
        } else {
          // Si el objeto 'user' no tiene ID, la sesión es inválida
          throw new Error("Datos de usuario inválidos en la sesión.");
        }
      } catch {
        // Si hay cualquier error al parsear o falta el id, limpiamos y redirigimos
        localStorage.clear();
        navigate("/login");
      }
    } else {
      // Si no hay item 'user', no hay sesión, redirigimos
      navigate("/login");
    }
  }, [navigate]);
  ...
  const fetchTickets = useCallback(async () => {
    ...
    setError(null);
    try {
      // Asumimos que tu helper apiFetch ya inyecta el authToken del localStorage
      const fetchedTickets = await apiFetch<TicketType[]>(`/tickets/municipio?user_id=${userId}`);
      setTickets(fetchedTickets || []); // Aseguramos que sea un array
    } catch (err) {
  ...
  // Se dispara el fetch solo cuando tenemos un userId válido
  useEffect(() => {
    if (userId) {
      fetchTickets();
    }
...
// Interaces de Props para los sub-componentes
...
// --- Sub-componentes (sin cambios en su lógica interna) ---
