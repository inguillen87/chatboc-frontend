(function () {
  ...
  // Check data-shadow-dom attribute
  ...
  // If not using Shadow DOM, target the container ID instead of :host
  const selector = useShadow ? ":host" : `#${containerId}`;
  ...
  function applyDims(dims) {
    const host = useShadow ? container.shadowRoot.host : container;
